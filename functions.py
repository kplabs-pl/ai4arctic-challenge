...
def chart_cbar(ax, n_classes, chart, cmap='vridis'):
    ...
    cbar = plt.colorbar(mpl.cm.ScalarMappable(norm=norm, cmap=cmap), ticks=arranged, fraction=0.043, pad=0.049, ax=ax)
...
def compute_metrics(true, pred, charts, metrics):
    ...
    for chart in charts:
        if true[chart].ndim == 1 and pred[chart].ndim == 1:
            ...
            print(
                f"true and pred must be 1D numpy array, got {true['SIC'].ndim} and {pred['SIC'].ndim} dimensions with shape {true['SIC'].shape} and {pred.shape}, respectively"
            )
